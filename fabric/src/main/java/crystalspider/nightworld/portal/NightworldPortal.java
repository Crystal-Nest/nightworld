package crystalspider.nightworld.portal;
...
public class NightworldPortal {
  private static final AbstractBlock.ContextPredicate IS_VALID_FRAME_BLOCK = (state, world, pos) -> state.isOf(Blocks.CRYING_OBSIDIAN);
  ...
  public static Optional<NightworldPortal> getNewPortal(WorldAccess world, BlockPos pos, Direction.Axis axis) {
    return NightworldPortal.getOrEmpty(world, pos, areaHelper -> areaHelper.isValid() && areaHelper.foundPortalBlocks == 0, axis);
  }

  public static Optional<NightworldPortal> getOrEmpty(WorldAccess world, BlockPos pos, Predicate<NightworldPortal> validator, Direction.Axis axis) {
    Optional<NightworldPortal> optional = Optional.of(new NightworldPortal(world, pos, axis)).filter(validator);
    if (optional.isPresent()) {
      return optional;
    }
    Direction.Axis axis2 = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
    return Optional.of(new NightworldPortal(world, pos, axis2)).filter(validator);
  }

  public NightworldPortal(WorldAccess world, BlockPos pos, Direction.Axis axis) {
    this.world = world;
    this.axis = axis;
  ...
  private BlockPos getLowerCorner(BlockPos pos) {
    ...
    while (pos.getY() > i && NightworldPortal.validStateInsidePortal(this.world.getBlockState(pos.down()))) {
      pos = pos.down();
    }
  ...
  private int getWidth(BlockPos pos, Direction direction) {
    ...
    for (int i = 0; i <= 21; ++i) {
      ...
      if (!NightworldPortal.validStateInsidePortal(blockState)) {
  ...
  public static TeleportTarget getNetherTeleportTarget(ServerWorld destination, BlockLocating.Rectangle portalRect, Direction.Axis portalAxis, Vec3d offset, Entity entity, Vec3d velocity, float yaw, float pitch) {
    ...
    Vec3d vec3d3 = NightworldPortal.findOpenPosition(vec3d2, destination, entity, entityDimensions);
